...
class Save extends \Magento\Customer\Controller\AbstractAccount
{
    ...
    public function __construct(
        ...
        $this->_categoryFactory = $categoryFactory ?: \Magento\Framework\App\ObjectManager::getInstance()
    ...
    {
        var_dump('Marketplace Add Product Save function clicked!');
        $helper = $this->helper;
        ...
        if ($isPartner == 1) {
            ...
            // var_dump(count($wholedata["product"]["category_ids"]));
            // var_dump("category leave status");
            // var_dump($wholedata["product"]["category_ids"][0]);
            // $category = $this->_categoryFactory->create()->load($wholedata["product"]["category_ids"][0]);
            // // var_dump($category->getIsLeaf());
            // var_dump($category->hasChildren());
            ...
            // selected category validation checking
            $cateflag = false;
            if (count($wholedata['product']['category_ids']) == 2) {
                $category = $this->_categoryFactory->create()->load($wholedata["product"]["category_ids"][0]);
                $cateflag = !($category->hasChildren());
            }
            ...
            try {
                ...
                if ($productId != '' && $cateflag) {
                    ...
                    if (!$cateflag) {
                        $this->messageManager->addError('You must select only one sub category for each product!');
                    }
                    $this->getDataPersistor()->set('seller_catalog_product', $wholedata);
